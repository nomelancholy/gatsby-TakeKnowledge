...
트리는 기업의 조직도처럼 계층적인 관계성을 나타낼 때 사용하는 비선형 자료구조입니다. 트리는 노드와 노드를 연결하는 간선으로 구성됩니다.  트리의 가장 위에 있는 노드를 루트(root) 노드라고 하고 가장 끝에 있는 노드, 노드에 연결된 간선의 개수를 차수라고 하는데 차수가 0인 노드를 잎(Leaf) 노드라고 합니다. 루트 노드와 잎 노드를 제외한 노드는 모두 내부 노드라고 합니다.
